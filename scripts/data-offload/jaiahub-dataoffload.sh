...
# Any additional arguments ($4+) are passed through directly to rsync
# (e.g. --include=*.unb --exclude=* --remove-source-files)
extra_rsync_args=("${@:4}")

# don't specify jaia user for simulation localhost offloads, otherwise do so
if [[ "${bot_ip}" == "127.0.0.1" ]]; then
    nice -n 10 rsync -aP --info=progress2 --no-inc-recursive --timeout=15 "${extra_rsync_args[@]}" "${staging_dir}/" "${offload_dir}"
else
    nice -n 10 rsync -aP --info=progress2 --no-inc-recursive --timeout=15 "${extra_rsync_args[@]}" "jaia@${bot_ip}:${staging_dir}/" "${offload_dir}"
fi
