...
# systemd.py rewrites runtime.env entirely from its own arguments, so jaia_comms_mode -- and
# jaia_cloudhub_type, which is derived from it -- fall back to the --comms_links argparse default
# of "xbee" unless the existing value is passed back in below. On a CloudHub (hub 30) xbee is
# filtered out as an unsupported link, so the default leaves jaia_comms_mode empty and demotes
# jaia_cloudhub_type from primary to secondary, which in turn stops the primary-only services
# (Iridium) from being generated. A secondary CloudHub does legitimately have no links, and
# --comms_links is nargs="+" and so rejects an empty value, hence the guard.
...
=======
        ./systemd-local.sh ${jaiabot_systemd_type} --bot_index $jaia_bot_index --fleet_index $jaia_fleet_index --electronics_stack $jaia_electronics_stack --imu_type $jaia_imu_type --imu_install_type $jaia_imu_install_type --arduino_type $jaia_arduino_type --bot_type ${jaia_bot_type,,} --bot_vin "$jaia_bot_vin" --pam_connection_type ${jaia_pam_connection_type,,} $jaia_simulation --enable --motor_harness_type ${jaia_motor_harness_type,,} --camera_positions ${jaia_camera_positions_args} --additional_sensors ${jaia_additional_sensors_args} --temperature_sensor_type ${jaia_temperature_sensor_type,,} ${jaia_comms_links_flag}) || { echo "❌ Failed to install the $jaiabot_systemd_type systemd services, so this deploy is still running the previously installed code"; exit 1; }
>>>>>>> Stashed changes
