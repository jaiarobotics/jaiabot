...
command="goby zeromq publish jaiabot_power_board::mcu_command  jaiabot.sensor.protobuf.SensorRequest 'time: 0 mcu_command: ENTER_BOOTLOADER_MODE' --interprocess 'platform: \"${PLATFORM}\"'"
eval "$command"
sleep 1

# Stop the jaiabot_power_board service to prevent stray data from going across the line
...
if stm32flash-0.7/stm32flash -w "$BIN" -v -b "115200" -g "0x08000000" "/dev/power-board"; then
