...
- In a terminal, run python3 calibration_and_imu_issue_counter_to_excel.py <path/to/h5>.
...
- Add <-c> as a flag to combine files from the same day of testing. 
...
maximum_distance_off_desired_dive_hold_depth = 1 # In meters
maximum_time_at_incorrect_dive_hold_depth = 3 # In seconds
    ...
    for filename in file_list:
        with h5py.File(filename, 'r') as file:
            ...
            try:
                ...
                desired_depth = desired_depth.reindex(range(len(status_depth)))
                desired_depth.reset_index(drop=True, inplace=True)
                
                # print("Desired depth length: ", len(desired_depth))
                        ...
                        while mission_state[i] == 125:
                            if desired_depth[i] != 0 and not np.isnan(desired_depth[i]):
                                ...
                                if abs(status_depth[i] - desired_depth[i]) > maximum_distance_off_desired_dive_hold_depth:
                                    ...
                                    if time_at_incorrect_depth > maximum_time_at_incorrect_dive_hold_depth:
                                        holds_dive += 1
                                        time_at_incorrect_depth = 0
                                else:
                                    time_at_incorrect_depth = 0
                ...
                if(instance_bot_id in sd):
                    ...
                    # print(reaquire_gps_lengths.mean())
