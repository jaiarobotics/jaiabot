...
def main():
    ...
    node_ip_result = subprocess.run(f'jaia ip --query_type addr --node_type {args.type} --ip_net wlan --fleet_id {fleet_cfg_json["fleet"]} --node_id {args.id} --ip_version ipv4', shell=True, capture_output=True, text=True)
    fleet_cfg_json['this']['ip'] = node_ip_result.stdout.strip()

    gateway_ip_result = subprocess.run(f'jaia ip --query_type addr --node_type gateway --ip_net wlan --fleet_id {fleet_cfg_json["fleet"]} --ip_version ipv4', shell=True, capture_output=True, text=True)
