...
class StormManager : public goby::zeromq::MultiThreadApplication<config::StormManager>
{
  public:
    ...
    // fsync'd record of the last state reached, so a lost log tail can't hide where a
    // wake actually ended
    void record_last_state(protobuf::StormMissionState state, const std::string& note = "");
