      ...
      AppMethodsAccess<InMission>
{
    ...
    void resume_after_srp_egress()
    {
        ...
        protobuf::MissionPlan::Segment active_seg =
            this->machine().mission_plan().segments(active_seg_index_);
        ...
        if (active_seg_index_ + 1 >= this->machine().mission_plan().segments_size())
        {
            glog.is_debug1() && glog << "No more lanes and no more segments, go to final waypoint"
                                     << std::endl;
            set_goal_index_to_final_goal();
            return;
        }

        protobuf::MissionPlan::Segment next_seg =
            this->machine().mission_plan().segments(active_seg_index_ + 1);
