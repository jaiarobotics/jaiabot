...
{
    using TrinketTempThread = goby::middleware::io::UDPPointToPointThread<jaiabot::groups::trinket_udp_in, jaiabot::groups::trinket_udp_out>;
    ...
    if (!cfg().is_in_sim() || cfg().test_hardware_in_sim())
    {
        ...
        if (cfg().trinket().trinket_harness_info_type() != jaiabot::protobuf::TrinketHarnessInfoType::NONE)
        {
            launch_thread<TrinketTempThread>(cfg().udp_config());
            launch_thread<TrinketStatusThread>(cfg().trinket());
        }
    }
