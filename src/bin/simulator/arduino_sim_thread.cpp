...
void jaiabot::apps::ArduinoSimThread::handle_arduino_command(
    const jaiabot::protobuf::ArduinoCommand& arduino_command)
{
    glog.is_debug2() && glog << group(thread_name())
                             << "Received Arduino Command: " << arduino_command.ShortDebugString()
                             << std::endl;
