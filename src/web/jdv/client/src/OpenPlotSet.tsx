    ...
    render() {
        const plotSetItems = Object.keys(this.state.plotSets).map((name, index) => {
            ...
            const row = (
                <div
                    key={key}
                    onClick={this.didClickPlotSet.bind(this, name)}
                    className="padded listItem horizontal flexbox"
                >
                    <div className="growable">{name}</div>
                    <button
                        title="Delete"
                        className="button"
                        onClick={this.didClickDeletePlotSet.bind(this, name)}
                    >
                        <Icon
                            path={mdiTrashCan}
                            size={1}
