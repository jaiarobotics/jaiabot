...
/**
 * Produces the dialog box that appears when clicking on the go to rally button.
 ...
 */
export function GoToRallyDialog(props: DialogProps) {
    /**
    ...
    const generateSubMessage = (disabledCode: DisabledCodes) => {
        ...
        if (disabledCode === DisabledCodes.NONE) {
            subMessage += `Send Bot${botIDs.length > 1 ? "s: " : ": "}`;
        } else {
            subMessage += `Cannot send to Bot${botIDs.length > 1 ? "s: " : ": "}`;
        }
 ...
 * For a confirmation dialog, the buttons will be Cancel and Rally Bots.
    ...
    if (numReadyBots > 0) {
        return (
            <div className="dialog-button-row">
                ...
                >
                    {`${numReadyBots > 1 ? "Rally Bots" : "Rally Bot"}`}
                </button>
            </div>
        );
    }
