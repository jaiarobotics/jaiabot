...
const equirectangular_to_mercator = (input: number[]) =>
    getTransform(equirectangular, mercator)(input, undefined, undefined)
const mercator_to_equirectangular = (input: number[]) =>
    getTransform(mercator, equirectangular)(input, undefined, undefined)
...
export default class CommandControl extends React.Component {
    ...
    botsLayerCollection: OlCollection<OlVectorLayer<OlVectorSource>> = new OlCollection([], {
        unique: true
    })
    ...
    hubsLayerCollection: OlCollection<OlVectorLayer<OlVectorSource>> = new OlCollection([], {
        unique: true
    })
    ...
    constructor(props: Props) {
        ...
        this.coordinate_to_location_transform = (coordinate: number[]) => {
            return getTransform(map.getView().getProjection(), equirectangular)(
                coordinate,
                undefined,
                undefined
            )
        ...
        this.surveyPolygonInteraction.on('drawstart', (evt: DrawEvent) => {
            this.setState({
            ...
            surveyPolygonlistener = evt.feature.on('change', (evt2: BaseEvent) => {
                ...
                if (turfPolygon.geometry.coordinates[0].length > 500) {
                    ...
                    if (missionPlanningGridTurf.features.length > 0) {
                        ...
                        if (missionPlanningGridTurfRotated.features.length > 0) {
                            ...
                            offsetLines.push(ol)
                            bot_list.forEach((bot) => {
                                ol = turf.lineOffset(ol, this.state.missionParams.spacing, {
                                    units: 'meters'
                                })
                                offsetLines.push(ol)
                            })
                            ...
                            c.coordinates.forEach((coord) => {
                                d.push(
                                    (
                                        format
                                            .readFeature(turf.explode(coord as any).features[0], {
                                                dataProjection: 'EPSG:4326',
                                                featureProjection: 'EPSG:3857'
                                            })
                                            .getGeometry() as any
                ...
                $('#missionStatRallyStartDistance').text(
                    this.state.missionParams.sp_rally_start_dist
                )
                $('#missionStatRallyFinishDistance').text(
                    this.state.missionParams.sp_rally_finish_dist
                )
        ...
        this.surveyPolygonInteraction.on('drawend', (evt: DrawEvent) => {
            this.setState({
                surveyPolygonChanged: true,
                mode: Mode.MISSION_PLANNING,
    ...
    componentDidMount() {
        ...
        function round(value: any, precision: number): any {
            ...
            if (typeof value === 'object' && value !== null)
                return Object.fromEntries(
                    Object.entries(value).map(([k, v]) => [k, round(v, precision)])
                )
        ...
        this.surveyExclusionsInteraction.on('drawstart', (evt: DrawEvent) => {
            this.setState({
                surveyExclusions: null
            })
            this.updateMissionLayer()
        ...
        this.surveyLinesInteraction.on('drawstart', (evt: DrawEvent) => {
            this.setState({
                missionPlanningFeature: null
            })
            this.updateMissionLayer()
            ...
            surveyLineslistener = evt.feature.on('change', (evt2) => {
                ...
                if (stringCoords[0].length >= 2) {
                    ...
                    if (currentLineLength <= maxLineLength - Number(missionParams.spacing) / 1000) {
                        ...
                        missionParams.sp_rally_start_dist = round(
                            turf.distance(
                        ...
                        missionParams.sp_rally_finish_dist = round(
                            turf.distance(
        ...
        this.surveyLinesInteraction.on('drawend', (evt: DrawEvent) => {
    ...
    updateBotsLayer() {
        ...
        for (const botId in bots) {
            ...
            botFeature.set(
                'remoteControlled',
                bot.mission_state?.includes('REMOTE_CONTROL') || false
            )
    ...
    selectBots(bot_ids: number[]) {
        ...
        bot_ids = bot_ids.map((bot_id) => {
            return Number(bot_id)
        })
    ...
    selectHubs(hub_ids: number[]) {
        hub_ids = hub_ids.map((hub_id) => {
            return Number(hub_id)
        })
    ...
    changeMissions(func: (runList: MissionInterface) => void, previousRunList: MissionInterface) {
        ...
        if (previousRunList != null || previousRunList != undefined) {
    ...
    returnToHome() {
        if (!this.state.homeLocation) {
            alert(
                'No Home location selected.  Click on the map to select a Home location and try again.'
            )
            return
        }
    ...
    weAreInControl() {
        return (
            this.podStatus.controllingClientId == this.api.clientId ||
            this.podStatus.controllingClientId == null
        )
    }
    ...
    takeControl() {
        ...
        return confirm(
            'WARNING:  Another client is currently controlling the team.  Click OK to take control of the team.'
        )
    }
    ...
    render() {
        ...
        switch (detailsBoxItem?.type) {
            case 'hub':
                detailsBox = HubDetailsComponent(
                    hubs?.[this.selectedHubId()],
                    this.api,
                    closeDetails,
            ...
            case 'bot':
                ...
                //**********************
                detailsBox = BotDetailsComponent(
                    bots?.[this.selectedBotId()],
                    hubs?.[0],
                    this.api,
                    this.state.runList,
                    closeDetails,
        ...
        function closeOtherViewControlWindows(openPanel: string) {
            const panels = [
                ...
                {
                    name: 'measureTool',
                    closeFunction: () => self.setState({ measureActive: false })
                },
        ...
        return (
            <div id='axui_container' className={containerClasses}>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (
                                    ...
                                    this.setState({
                                        surveyPolygonActive: true,
                                        mode: Mode.MISSION_PLANNING
                                    })
                                    if (this.state.missionParams.mission_type === 'polygon-grid')
                                        this.changeInteraction(
                                            this.surveyPolygonInteraction,
                                            'crosshair'
                                        )
                                    if (this.state.missionParams.mission_type === 'editing')
                                        this.changeInteraction(this.selectInteraction(), 'grab')
                                    if (this.state.missionParams.mission_type === 'lines')
                                        this.changeInteraction(
                                            this.surveyLinesInteraction,
                                            'crosshair'
                                        )
                                    if (this.state.missionParams.mission_type === 'exclusions')
                                        this.changeInteraction(
                                            this.surveyExclusionsInteraction,
                                            'crosshair'
                                        )
                                    ...
                                    info(
                                        'Please place a green and red rally point before using this tool'
                                    )
                                }
                            }}
                        >
                            <FontAwesomeIcon
                                icon={faEdit as any}
                                title='Edit Optimized Mission Survey'
                            />
                        </Button>
                    )}
                ...
                <div id='botsDrawer'>
                    ...
                    <div
                        id='jaiabot3d'
                        style={{ zIndex: '10', width: '50px', height: '50px', display: 'none' }}
                    ></div>
                </div>
            ...
            for (const run_key in this.state.runList.runs) {
                if (this.state.runList.runs[run_key].assigned == -1) {
                    ...
                    delete this.state.runList.botsAssignedToRuns[
                        this.state.runList.runs[run_key].assigned
                    ]
                    ...
                    this.state.runList.botsAssignedToRuns[
                        this.state.runList.runs[run_key].assigned
                    ] = this.state.runList.runs[run_key].id
        ...
        if (bot_list.length >= 3) {
            ...
            bot_list.forEach((bot) => {
                rhumbDestinationPoints.push(
                    turf.coordAll(
                        turf.rhumbDestination(turf.point(center), nextRadius, rotationAngle - 90)
                    )
                )
    ...
    updateMissionPlansFromMissionPlanningGrid() {
        ...
        const rallyStartPoints = this.findRallySeparation(
            ...
            this.state.rallyPointGreenLocation,
            this.state.missionParams.orientation,
            this.state.missionParams.rally_spacing
        )
    ...
    deleteSingleRun() {
        ...
        const runId = runList.botsAssignedToRuns[selectedBotId]
            ? runList.botsAssignedToRuns[selectedBotId]
            : -1
    ...
    commandDrawer() {
        const element = (
            <div id='commandsDrawer'>
                ...
                <Button
                    className='button-jcc'
                    id='setRallyPointGreen'
                    onClick={this.setRallyPointGreenClicked.bind(this)}
                >
                ...
                <Button
                    className='button-jcc'
                    id='goToRallyGreen'
                    onClick={this.goToRallyGreen.bind(this)}
                >
                ...
                <Button
                    className='button-jcc'
                    id='setRallyPointRed'
                    onClick={this.setRallyPointRedClicked.bind(this)}
                >
                ...
                <Button
                    className='button-jcc'
                    id='goToRallyRed'
                    onClick={this.goToRallyRed.bind(this)}
                >
                ...
                <Button
                    id='missionStartStop'
                    className='button-jcc stopMission'
    ...
    undoButton() {
        return (
            <Button
                className={'globalCommand' + ' button-jcc'}
                onClick={this.restoreUndo.bind(this)}
            >
    ...
    goToRallyGreen(evt: UIEvent) {
        ...
        if (!this.state.rallyPointGreenLocation) {
            alert(
                'No green rally point selected.  Click on the map to select a green rally location and try again.'
            )
            return
        }

        if (
            this.areBotsAssignedToRuns() &&
            !confirm(
                'Going to the green rally point will delete all runs in the mission. If the current mission is saved, select OK'
            )
        ) {
            return
        }
    ...
    goToRallyRed(evt: UIEvent) {
        ...
        if (!this.state.rallyPointRedLocation) {
            alert(
                'No red rally point selected.  Click on the map to select a red rally location and try again.'
            )
            return
        }
        ...
        if (
            this.areBotsAssignedToRuns() &&
            !confirm(
                'Going to the red rally point will delete all runs in the mission. If the current mission is saved, select OK'
            )
        ) {
            return
        }
    ...
    activateAllClicked(evt: UIEvent) {
        if (
            !this.takeControl() ||
            !confirm('Click the OK button to run a system check for all active bots:')
        )
            return
    ...
    nextTaskAllClicked(evt: UIEvent) {
        if (
            !this.takeControl() ||
            !confirm('Click the OK button to run the next task for all active bots:')
        )
            return
    ...
    recoverAllClicked(evt: UIEvent) {
        if (!this.takeControl() || !confirm('Click the OK button to recover all active bots:'))
            return
