            ...
            if (taskPacket.type == 'DIVE') {
                ...
                let iconInfoStyle = new OlStyle({
                    text: new OlText({
                        ...
                        text:
                            `Depth (m): ` +
                            divePacket.depth_achieved +
                            '\nDiveRate (m/s): ' +
                            divePacket.dive_rate,
                        scale: 1,
                ...
                if (
                    ...
                    let iconInfoStyle = new OlStyle({
                        text: new OlText({
                            ...
                            text:
                                `Duration (s): ` +
                                driftPacket.drift_duration +
                                '\nDirection (deg): ' +
                                task_calcs.driftDirection.toFixed(2) +
                                '\nSpeed (m/s): ' +
                                task_calcs.driftSpeed.toFixed(2),
                            scale: 1,
                            fill: new OlFillStyle({ color: 'white' }),
                            backgroundFill: new OlFillStyle({ color: 'black' }),
                            textAlign: 'end',
    ...
    _pollTaskPackets() {
        jaiaAPI.getTaskPackets().then((taskPackets: TaskPacket[]) => {
            ...
            const taskPacketFeatures = taskPackets
                .map((taskPacket) => {
                    return createTaskPacketFeatures(this.map, taskPacket)
                })
                .flat()
