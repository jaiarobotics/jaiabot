...
const commands: { [key: string]: CommandInfo } = {
  ...
  retryDataOffload: {
    ...
    humanReadableAvailable:
      "POST_DEPLOYMENT__IDLE, POST_DEPLOYMENT__WAIT_FOR_MISSION_PLAN",
    humanReadableNotAvailable: "",
  },
  shutdown: {
...
function issueCommandForHub(
  api: JaiaAPI,
  hub_id: number,
  commandForHub: CommandInfo
) {
...
function disableButton(
  ...
  const disableState =
    ...
    `${
      humanReadableNotAvailable !== ""
        ? "States Not Available: " + humanReadableNotAvailable + "\n"
        : ""
    }`;
 ...
 * @returns boolean
 */
function disableClearRunButton(
  bot: PortalBotStatus,
  ...
  if (!mission?.botsAssignedToRuns[bot.bot_id]) {
    disableInfo.disableMessage =
      "Cannot perform this action because there is no run to delete";
    disableInfo.isDisabled = true;
  }
...
function disablePlayButton(
  ...
  if (disableButton(command, missionState).isDisabled) {
    disableInfo.disableMessage += disableButton(
      command,
      missionState
    ).disableMessage;
    disableInfo.isDisabled = true;
  ...
  if (bot.health_state === "HEALTH__FAILED") {
    disableInfo.disableMessage +=
      "The command: " +
      command.commandType +
      ' cannot be sent because the bot because it has a health state of "HEALTH__OK" or "HEALTH__DEGRADED\n';
    disableInfo.isDisabled = true;
  }
...
function healthRow(bot: BotStatus, allInfo: boolean) {
  ...
  let healthStateElement = (
    <div className={healthClassName}>{bot.health_state}</div>
  );
...
export interface BotDetailsProps {
  ...
  setDetailsExpanded: (
    section: keyof DetailsExpandedState,
    expanded: boolean
  ) => void;
  isRCModeActive: (botId: number) => boolean;
  setRcMode: (botId: number, rcMode: boolean) => void;
...
export function BotDetailsComponent(props: BotDetailsProps) {
  ...
  if (bot.repeat_index != null) {
    ...
    if (bot.active_mission_plan?.repeats != null) {
      repeatNumberString =
        repeatNumberString + ` of ${bot.active_mission_plan?.repeats}`;
    }
  }
  ...
  let dataOffloadButton = (
    <Button
      className={
        disableButton(commands.recover, missionState).isDisabled ||
  ...
  return (
    <React.Fragment>
      <div id="botDetailsBox">
        ...
        <div
          id="botDetailsAccordionContainer"
          className="accordionParentContainer"
        >
              ...
              <AccordionDetails className="botDetailsCommands">
                ...
                <Button
                  className={`
                                        ...
                                        ${
                                          props.isRCModeActive(bot?.bot_id)
                                            ? "rc-active"
                                            : "rc-inactive"
                                        }
                                        `}
                      ...
                      <table>
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                              {bot.attitude?.course_over_ground?.toFixed(prec)}
                            </td>
                          </tr>
                        </tbody>
                      </table>
...
export function HubDetailsComponent(props: HubDetailsProps) {
  ...
  if (hub.linux_hardware_status?.processor?.loads?.one_min != undefined) {
    loadAverageOneMin =
      hub.linux_hardware_status?.processor?.loads?.one_min.toFixed(2);
  } else {
  ...
  if (hub.linux_hardware_status?.processor?.loads?.five_min != undefined) {
    loadAverageFiveMin =
      hub.linux_hardware_status?.processor?.loads?.five_min.toFixed(2);
  } else {
    loadAverageFiveMin = "N/A";
  }

  if (hub.linux_hardware_status?.processor?.loads?.fifteen_min != undefined) {
    loadAverageFifteenMin =
      hub.linux_hardware_status?.processor?.loads?.fifteen_min.toFixed(2);
  } else {
  ...
  if (hub.linux_hardware_status?.wifi?.link_quality_percentage != undefined) {
    linkQualityPercentage =
      hub.linux_hardware_status?.wifi?.link_quality_percentage;
  }

  return (
    <div id="hubDetailsBox">
      <div
        id="hubDetailsAccordionContainer"
        className="accordionParentContainer"
      >
        ...
        <div id="hubDetailsAccordionContainer">
                ...
                <Button
                  ...
                  onClick={() => {
                    issueCommandForHub(
                      api,
                      hub.hub_id,
                      commandsForHub.restartServices
                    );
                  }}
                >
                  <Icon path={mdiRestart} title="Restart Services" />
          ...
          <ThemeProvider theme={makeAccordionTheme()}>
            <Accordion
              ...
              onChange={(event, expanded) => {
                setDetailsExpanded("links", expanded);
              }}
              className="accordionContainer"
            >
              <AccordionSummary
              ...
              <AccordionDetails>
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (fleetId != undefined && !Number.isNaN(hubId)) {
