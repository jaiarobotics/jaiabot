...
const commands: { [key: string]: CommandInfo } = {
    ...
    recover: {
        ...
        description: 'Recover',
        statesAvailable: [/^IN_MISSION__UNDERWAY__RECOVERY__STOPPED$/]
    },
    retryDataOffload: {
        commandType: CommandType.RETRY_DATA_OFFLOAD,
        description: 'Retry Data Offload for',
        statesAvailable: [/^POST_DEPLOYMENT__IDLE$/, /^POST_DEPLOYMENT__WAIT_FOR_MISSION_PLAN$/]
    },
    shutdown: {
...
export function BotDetailsComponent(
    bot: PortalBotStatus,
    hub: PortalHubStatus,
    api: JaiaAPI,
    ...
    let dataOffloadButton = (
        <Button
            className={disableButton(commands.recover, mission_state).class + ' button-jcc'}
            disabled={disableButton(commands.recover, mission_state).isDisabled}
            onClick={() => {
                issueCommand(api, bot.bot_id, commands.recover)
            }}
        >
    ...
    if (!dataOffloadStatesAvailable.test(mission_state)) {
        dataOffloadButton = (
            <Button
                className={
                    disableButton(commands.retryDataOffload, mission_state).class + ' button-jcc'
                }
                disabled={disableButton(commands.retryDataOffload, mission_state).isDisabled}
                onClick={() => {
                    issueCommand(api, bot.bot_id, commands.retryDataOffload)
                }}
            >
            ...
            <div id='botDetailsBox'>
                <div className='botDetailsHeading'>
                    ...
                    <div className='botDetailsToolbar'>
                        <Button
                            className={
                                disableButton(commands.stop, mission_state).class +
                                ' button-jcc stopMission'
                            }
                            disabled={disableButton(commands.stop, mission_state).isDisabled}
                            onClick={() => {
                                issueCommand(api, bot.bot_id, commands.stop)
                            }}
                        >
                        ...
                        <Button
                            className={
                                disableButton(commands.play, mission_state).class + ' button-jcc'
                            }
                            disabled={disableButton(commands.play, mission_state).isDisabled}
                            onClick={() => {
                                issueRunCommand(api, runMission(bot.bot_id, mission), bot.bot_id)
                            }}
                        >
                        ...
                        <Button
                            className={
                                disableClearMissionButton(bot.bot_id, mission).class + ' button-jcc'
                            }
                ...
                <div className='accordionContainer'>
                    <Accordion
                        ...
                        onChange={() => {
                            detailsDefaultExpanded('quickLook')
                        }}
                        className='accordion'
                    >
                        <AccordionSummary
                    ...
                    <Accordion
                        ...
                        onChange={() => {
                            detailsDefaultExpanded('commands')
                        }}
                        className='accordion'
                    >
                        <AccordionSummary
                        ...
                        <AccordionDetails>
                            ...
                            <Button
                                className={
                                    disableButton(commands.active, mission_state).class +
                                    ' button-jcc'
                                }
                                disabled={disableButton(commands.active, mission_state).isDisabled}
                                onClick={() => {
                                    issueCommand(api, bot.bot_id, commands.active)
                                }}
                            >
                                <Icon
...

                            {
                                <Button
                                    className={
                                        disableButton(commands.rcMode, mission_state).class +
                                        ' button-jcc'
                                    }
                                    disabled={
                                        disableButton(commands.rcMode, mission_state).isDisabled
                                    }
                                    onClick={() => {
                                        issueRCCommand(api, runRCMode(bot), bot.bot_id)
                                    }}
                                >
                            ...
                            <Button
                                className={
                                    disableButton(commands.nextTask, mission_state).class +
                                    ' button-jcc'
                                }
                                disabled={
                                    disableButton(commands.nextTask, mission_state).isDisabled
                                }
                                onClick={() => {
                                    issueCommand(api, bot.bot_id, commands.nextTask)
                                }}
                            >
                            ...
                            <Accordion
                                ...
                                onChange={() => {
                                    detailsDefaultExpanded('advancedCommands')
                                }}
                            ...
                            >
                                ...
                                <AccordionDetails>
                                    <Button
                                        className={
                                            disableButton(commands.shutdown, mission_state).class +
                                            ' button-jcc'
                                        }
                                        disabled={
                                            disableButton(commands.shutdown, mission_state)
                                                .isDisabled
                                        }
                                        onClick={() => {
                                            if (
                                    ...
                                    <Button
                                        className={
                                            disableButton(commands.reboot, mission_state).class +
                                            ' button-jcc'
                                        }
                                        disabled={
                                            disableButton(commands.reboot, mission_state).isDisabled
                                        }
                                        onClick={() => {
                                            if (
                                                bot.mission_state ==
                                    ...
                                    <Button
                                        className={
                                            disableButton(commands.restartServices, mission_state)
                                                .class + ' button-jcc'
                                        }
                                        disabled={
                                            disableButton(commands.restartServices, mission_state)
                                    ...
                                    <table>
                                        <tbody>
                                            <tr>
                                                <td>Heading</td>
                                                <td>
                                                    {formatAttitudeAngle(bot.attitude?.heading)}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>Pitch</td>
                ...
                >
                    ...
                    <AccordionDetails>
                        <Button
                            ...
                            onClick={() => {
                                issueCommandForHub(api, hub.hub_id, commandsForHub.shutdown)
                            }}
                        >
                            <Icon path={mdiPower} title='Shutdown' />
                        </Button>
                        <Button
                            className={' button-jcc'}
                            onClick={() => {
                                issueCommandForHub(api, hub.hub_id, commandsForHub.reboot)
                            }}
                        >
                            <Icon path={mdiRestartAlert} title='Reboot' />
                        </Button>
                        <Button
                            className={' button-jcc'}
                            onClick={() => {
                                issueCommandForHub(api, hub.hub_id, commandsForHub.restartServices)
                            }}
                        >
                            <Icon path={mdiRestart} title='Restart Services' />
                        </Button>
