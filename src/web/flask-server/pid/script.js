...
class TabbedSections {
  constructor(tabbedSections, activeIndex) {
    ...
    for (let i in tabbedSections) {
      let tabbedSection = tabbedSections[i]

      tabbedSection.button.onclick = function(e) {
        for (let j in tabbedSections) {
          let otherTabbedSection = tabbedSections[j]
    ...
    this.tabbedSections[activeIndex].button.onclick()
  }
}
  ...
  switch (key) {
    ...
    case 'KeyS':
      if (DeadMansSwitch.on) {
        slider = throttleTabbedSections.activeIndex == 0 ? throttleSlider : speedSlider
        slider.decrement()
      }
      break
    case 'KeyW':
...
function sendVisibleCommand() {
  ...
  if (DeadMansSwitch.on) {
    ...
    // Throttle
    console.log(typeof(throttleTabbedSections.activeIndex))
