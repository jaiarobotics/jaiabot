...
module.exports = class RectangleSurveyGeometry {
  ...
  getFormations() {
    const centerLeftPoint = getCoordsFromHeadingAndDistance(this.center, this.heading - 90, this.width / 2.0);
    const bottomLeftPoint = getCoordsFromHeadingAndDistance(centerLeftPoint, this.heading + 180, this.height / 2.0);
    const formations = [];
    for (let i = 0; i < this.formationCount; i += 1) {
      const origin = getCoordsFromHeadingAndDistance(bottomLeftPoint, this.heading + 90, this.horizSeparation * i);
      const formation = new FormationGeometry(origin, this.heading, this.vertSeparation, 'line', this.botCount);
