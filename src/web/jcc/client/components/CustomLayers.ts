...
import { GeoTIFF, GeoTIFFImage, fromArrayBuffer,
         ReadRasterResult, TypedArrayWithDimensions } from 'geotiff';
...
export class CustomLayerGroupFactory {
    ...
    async createCustomLayerGroup(): Promise<void> {
        ...
        try {
            ...
            console.log("Error while fetching GeoTIFFs list: ", ex);
            return;
        }
        ...
        const layerPromises = sortedGeoTiffs.map(geoTiff => limit(() => this.createGeoTiffLayer(geoTiff)));
        ...
        loadedLayers.forEach(layer => customOlLayerGroup.getLayers().push(layer));
    ...
    sortGeoTiffNamesAndMakeUnique(geoTiffs: GeoTiffMetadata[]): GeoTiffMetadata[] {
        ...
        const keyedGeoTiffs = geoTiffs.reduce((accum, geoTiffItem) => {
            if (geoTiffItem.error) {
                console.log(`Warning: ignoring ${geoTiffItem.filename} due to this JSON error: ${geoTiffItem.error}
    ...
    async createGeoTiffLayer(geoTiffMd: GeoTiffMetadata): Promise<BaseLayer> {
        const imageOverlay: ImageLayer<Static> = new ImageLayer({
            properties: {
        ...
        imageOverlay.once('change:visible', (event) => this.loadGeoTiffSource(imageOverlay, geoTiffMd));
    ...
    async loadGeoTiffSource(imageOverlay: ImageLayer<Static>, geoTiffMd: GeoTiffMetadata) {
        if (!imageOverlay.getLayerState().visible) {
        ...
        if (!geoTiffMd.proj4 && !geoTiffMd.epsg) {
            console.log(`Warning: custom overlay ${geoTiffMd.filename} is missing its projection; it likely will not display.
        ...
        if (!geoTiffMd.bands) {
            ...
            const extent: number[] = [ 0, 0, 0, 0 ];
            if (!geoTiffMd.cornerCoordinates) {
                console.log(`Warning: custom overlay ${geoTiffMd.filename} is missing its extent; it likely will not display.
                Please add cornerCoordinate information to the metadata JSON file.`)
            } else {
                extent[0] = geoTiffMd.cornerCoordinates.lowerLeft?.[0];
                extent[1] = geoTiffMd.cornerCoordinates.lowerLeft?.[1];
                extent[2] = geoTiffMd.cornerCoordinates.upperRight?.[0];
                extent[3] = geoTiffMd.cornerCoordinates.upperRight?.[1];
                const hasNonNumber = extent.some((el: any) => isNaN(parseFloat(el)) || el == null);
                if (hasNonNumber) {
                    console.log(`Warning: custom overlay ${geoTiffMd.filename} has an incomplete extent; it likely will not display.
            ...
            // Load in a GeoTIFF
            const {canvas, extent} = await this.loadGeoTiff(imageOverlay, geoTiffMd);
            if (!canvas || !extent) {
                return;  // Could not load the image, so we can't create a source from it.
            }
            // Note that I am deeply unsettled by having to use toDataURL(), which causes a temporary
            // explosion of memory usage because DataURLs are several times more voluminous.
            source = new Static({
    ...
    // Grab the extent from the GeoTIFF then Bounce the band data into a canvas.
    // Supports RBG and single band (grayscale) rasters.
    // Also supports Min/Max limits, pixel inversion, and gray-scale colorization.
    async loadGeoTiff(imageOverlay: ImageLayer<Static>, geoTiffMd: GeoTiffMetadata): Promise<LoadedGeoTiff> {
        ...
        try {
            ...
            console.log("Error while fetching GeoTIF: ", ex);
            return {canvas: null, extent: null};
        }
        ...
        const extent: number[] = image.getBoundingBox();

        const canvas: HTMLCanvasElement = document.createElement("canvas");
        canvas.width = width;
        canvas.height = height;
        const context: CanvasRenderingContext2D = canvas.getContext("2d");
        const data: ImageData = context.createImageData(width, height);
        ...
        if (geoTiffMd.isRGB && geoTiffMd.bands.length >= 3) {
            const raster: ReadRasterResult = await image.readRGB() as TypedArrayWithDimensions;

            let op = 0;
            const bands = geoTiffMd.bands;
            const noRGBData = geoTiffMd.noDataValueRGB;
            for (let ip = 0; ip < raster.length; ip += 3) {
                rgb[op + 0] = raster[ip + 0];
                rgb[op + 1] = raster[ip + 1];
                rgb[op + 2] = raster[ip + 2];
                rgb[op + 3] = ((raster[ip + 0] == noRGBData?.[0] &&
                            ...
                            raster[ip + 2] == bands[2].noDataValue || (isNaN(raster[ip + 2]) && bands[2].noDataValue === "NaN")) ? 0 : alpha;
                if (invert) {
            ...
            const raster: TypedArrayWithDimensions = rasters[0] as any;

            const displayMin = geoTiffMd.band.displayMin;
            const displayMax = geoTiffMd.band.displayMax;

            const scale = 255.0 / (displayMax - displayMin);
            const chromaScale = geoTiffMd.displayOptions.chromaScale;
            const chromaScaleEffective = chromaScale && chroma.scale(chromaScale).mode('hsl');

            let op = 0;
            for (let ip = 0; ip < raster.length; ip++) {
                const rasterValue = raster[ip];
                const isNoData = (isNaN(rasterValue) && geoTiffMd.band.noDataValue === "NaN" ||
                                rasterValue == geoTiffMd.band.noDataValue);
                let level = 0;
                if (!isNoData) {
