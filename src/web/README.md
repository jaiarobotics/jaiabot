...
The Jaia web source code is located in the `src/web` directory. In this directory is a script called `run.sh`, which will build and launch JCC. The app will then be accessible from your browser.

When using the `run.sh` script, JCC will be built and run using the `Development` build mode. This means that the code will not be minified, and the source directories will be watched (monitored for any changes). If any changes are detected, the app will be rebuilt so that you may refresh your browser and see the code changes.
...
`JCC` and `JDV` are built using the `webpack` package. Configuration for `webpack` can be found in the `src/web/webpack.config.js` file. Using this file, `webpack` will build `JCC` using either the `development` or `production` mode.
...
The `webpack.config.js` file contains four different webpack configuration objects:
...
When invoked, the correct options are chosen depending on the build mode provided, and `JCC` is built.
