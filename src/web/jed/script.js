...
class Slider {
    constructor(
        ...
        if (orientation == vertical) {
            let centerButtonHTML = showCenterButton
                ? `<button class="sliderCenter" id="` + name + `Center">🎯</button>`
                : ``;
            let valueHTML = showValue
                ? `<div class="sliderName">` +
                  label +
                  `</div><div class="sliderValue" id="` +
                  name +
                  `Value"></div>`
                : ``;
      ...
      `;
            ...
            let centerButtonHTML = showCenterButton
                ...
                : ``;
            let valueHTML = showValue
                ? `<div class="sliderTop"><div class="sliderName">` +
                  label +
                  `</div><div class="sliderValue" id="` +
                  name +
                  `Value">0</div></div>`
...
const diveManualSlider = new Slider(
    vertical,
    "diveManual",
    0,
    100,
...
const divePIDSlider = new Slider(
    vertical,
    "divePID",
    0,
    100,
...
const throttleSlider = new Slider(
    ...
    "throttle",
    10,
    [DEAD_MANS_SWITCH_KEY, "KeyS"],
    [DEAD_MANS_SWITCH_KEY, "KeyW"],
    5,
);
...
const headingSlider = new AngleSlider(
    "headingWidget",
    "headingValue",
    0,
    360,
...
const timeoutSlider = new Slider(
    horizontal,
    "timeout",
    0,
    100,
      ...
      <div class="gains">
        <div class="gainRow">
          ...
          <input class="gain" type="text" id="` +
            name +
            `_Kp" name="fname" value="1"><br/>
        </div>
        <div class="gainRow">
          <div class="gainLabel">Ki</div>
          <input class="gain" type="text" id="` +
            name +
            `_Ki" name="fname" value="0"><br/>
        </div>
        <div class="gainRow">
          <div class="gainLabel">Kd</div>
          <input class="gain" type="text" id="` +
            name +
            `_Kd" name="fname" value="0"><br/>
        </div>
...
document.getElementById("LEDOnButton").addEventListener("click", LEDButtonOnClick);
...
document.getElementById("LEDOffButton").addEventListener("click", LEDButtonOffClick);
        ...
        switch (throttleTabbedSections.activeIndex) {
            case 0:
                if (el("throttleSlider").value > SAFE_BOT_SPEED) {
                    warningStatusInner =
                        "<label style='color:red; display: inline-block; font-size:24pt'>You are running above the safe speed level for the bot? (This may result in hardware failure)</label>";
                } else {
                    warningStatusInner = "";
                }
...
function updateBotSelectDropdown(bots) {
    ...
    let existingBotIds = Array.from(
        document.getElementById("botSelect").getElementsByTagName("option"),
