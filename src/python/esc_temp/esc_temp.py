...
import argparse
import socket
from jaiabot.messages.esc_temp_pb2 import EscTemp

parser = argparse.ArgumentParser(description='Read RPM from motor and publish it over UDP')
parser.add_argument('-p', '--port', dest='port', default=20006, help='Port to access motor readings')
        ...
        if line:
            ...
            try:
                ...
                if 'A0' in data and 'A4' in data:
                    ...
                    esc_data.a0_voltage = float(data['A0'])
                    esc_data.a4_voltage = float(data['A4'])

                    esc_data.a0_resistance = calculate_resistance(esc_data.a0_voltage)
                    esc_data.a4_resistance = calculate_resistance(esc_data.a4_voltage)

                    esc_data.a0_temperature = linear_interpolate(esc_data.a0_resistance, ohms_to_temperature)
                    esc_data.a4_temperature = linear_interpolate(esc_data.a4_resistance, ohms_to_temperature)

                    sock.sendto(esc_data.SerializeToString(), addr)
                    print(f"{timestamp} - A0: {esc_data.a0_voltage:.2f}V, {esc_data.a0_resistance:.2f}Ω, {esc_data.a0_temperature:.2f}°C; "
                          f"A4: {esc_data.a4_voltage:.2f}V, {esc_data.a4_resistance:.2f}Ω, {esc_data.a4_temperature:.2f}°C")
                else:
...
def main():
    port_thread = Thread(target=read_from_serial, name="port_thread", daemon=True)
    port_thread.start()
