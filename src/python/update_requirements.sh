...
docker run -w $(realpath ${script_dir}) -v ${jaia_root}:${jaia_root} -t gobysoft/jaiabot-ubuntu-amd64:26.04 \
       /bin/bash -c "set -euo pipefail; git config --global --add safe.directory ${jaia_root}; echo \"deb [signed-by=/etc/apt/keyrings/jaiabot.gpg] http://packages.jaia.tech/ubuntu/${repo}/${version}/ ${distro}/\" > /etc/apt/sources.list.d/jaiabot.list; apt update && apt install -y rsync python3-dev python3-venv jaiabot-python; rm -rf /usr/share/jaiabot/python/venv; ./build_venv.sh /tmp/jaia; source /tmp/jaia/venv/bin/activate; pip3 freeze --local  > requirements.txt"
