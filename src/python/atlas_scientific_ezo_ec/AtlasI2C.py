...
class Result:
    ...
    def __init__(self, error_code: int, response: str):
...
class AtlasI2C:
    ...
    def read(self, num_of_bytes=31):
        ...
        if is_valid:
            ...
            response =  str(''.join(char_list)).strip('\x00')
            return Result(int(error_code), response)
        else:
            return Result(int(error_code), None)
