...
2. Copy the overlay image to the /usr/share/jaiabot/overlays directory on the Jaia Hub
   (Question: do we need to document how to actually perform the copy?)
3. Run the scripts/geotiff_scan.sh script on your new overlay image to create a .meta.json file; this file contains metadata extracted from the overlay image.
   (Question: which directory on the Hub machine holds the script directory?)
...
- For the GeoTIFF format:
        ...
        - The custom overlay feature interprets a GeoTIFF with the first three bands labeled Red, Green, Blue as a single color image raster and ignores additional bands.
...
| isRGB             | boolean         | No       | *false* (i.e. display each band)          | If true, then treat the three bands as RGB pixel color.  The geotiff_scan.sh script will set isRGB *true* if your image has its first three bands named Red, Green, Blue; you can set isRGB to *false* if you really want to display the Red/Green/Blue bands separately.  Or, if your first three bands represent RGB values but are not labeled Red, Green, and Blue, then you can set isRGB to *true*. |
...
| colorInterpretation | string or null   | No       | *None*                                                       | The `geotiff_scan.sh` script extracts the `colorInterpretation` attribute from the GeoTIFF file. This attribute can be 'Red', 'Green', 'Blue', or 'Gray'. If the attribute is set to any other value, or is undefined/null, it will be treated as 'Gray'. The script performs two actions with this information:<br><br>1. It copies the value to this field.<br>2. It utilizes this value to set the 'isRGB' indicator within the base metadata.<br><br>Once these steps are completed, the `colorInterpretation` value is retained for informational purposes only and is not used in any further processing. |
