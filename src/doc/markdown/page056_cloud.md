...
A VirtualBox fleet has no such limit - its NAT network is local - so it mirrors the fleet WLAN whichever range the fleet is in, taking `fddd:7f2e:3258:{fleet}::/64` above 250 with IPv4 from the NAT network's DHCP for internet access only, exactly as a real fleet of that id does. The host SSH ports are forwarded to that IPv4 rather than to the fleet IPv6, because VirtualBox stores an IPv6 port forward but never activates it; `import_vms.sh` reserves each node's `vfleet_wlan` address in the NAT network's DHCP so the forward has a fixed address to aim at.
