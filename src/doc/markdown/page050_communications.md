...
The `Hub2HubData` message uses a `oneof contents` field to carry one of the following payloads:

- **`bot_status`**: A `BotStatus` received by the originating hub from one of its managed bots. This allows all hubs to display the status of bots managed by other hubs.
- **`task_packet`**: A `TaskPacket` received by the originating hub from one of its managed bots. This allows all hubs to record and display task data from bots managed by other hubs.
- **`command_for_bot`**: A `Command` sent by a JCC client connected to the originating hub, intended for a bot. This allows hubs that are in radio range of the target bot (but not the originating hub) to relay the command. Only locally-originated commands are forwarded (i.e., commands received from other hubs via `Hub2HubData` are not re-forwarded to avoid message loops).
- **`command_comms_result`**: A `CommandCommsResult` generated by the originating hub when it receives an acknowledgement or expiry for a command it sent to a bot. This allows JCC clients on other hubs to see the delivery status of commands they originated.

#### Data flow

When `jaiabot_hub_manager` receives a `Hub2HubData` message from another hub via the HUB2HUB link, it re-publishes it on the local interprocess transport (`jaiabot::groups::hub2hub_data`). The `jaiabot_web_portal` subscribes to this group and:

- Wraps `bot_status` and `task_packet` contents into a `PortalToClientMessage` and sends it to connected JCC clients.
- Forwards `command_for_bot` to the local `jaiabot_hub_manager` (via `jaiabot::groups::hub_command_full`) so the command can be relayed to the target bot if it is in range.
- Notifies JCC clients of the remote command via the `remote_command` field in `PortalToClientMessage`.
- Wraps `command_comms_result` into a `PortalToClientMessage` and sends it to connected JCC clients.

All `PortalToClientMessage` messages include a `hub_id` field that identifies the originating hub—either the local hub's ID (for locally-generated messages) or the remote hub's ID (for messages received via `Hub2HubData`). This allows JCC clients to distinguish which hub generated each message.
