...
When `jaiabot_hub_manager` receives a `Hub2HubData` message from another hub via the HUB2HUB link, it re-publishes it on the interprocess layer (`jaiabot::groups::hub2hub_data`). The `jaiabot_web_portal` subscribes to this group and:

- Wraps `bot_status`, `task_packet`, and `command_comms_result` into a `PortalToClientMessage` and sends it to JCC.
...
All `PortalToClientMessage` messages include a `hub_id` field that identifies the originating hub—either the local hub's ID (for locally-generated messages) or the remote hub's ID (for messages received via `Hub2HubData`). This allows JCC to distinguish which hub generated each message.
