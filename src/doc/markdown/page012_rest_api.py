...
#        anchor = re.sub(r'-+', '-', anchor)
    ...
    def add_subsections(self, subsections: Union["Section", Iterable["Section"], None]):
        if subsections is None:
            ...
            try:
                ...
                self.subsections.append(subsections)


def generate():
    document = Section(title='REST API')
    document.content = """
The Jaia REST API provides a way to query and command a JaiaBot fleet via web URL endpoints. These endpoints are directly generated from the APIRequest Protobuf message for input and APIResponse for output ([jaiabot/src/lib/messages/rest_api.proto](https://github.com/jaiarobotics/jaiabot/blob/3.y/src/lib/messages/rest_api.proto)). For information on units, look at the [DCCL extensions](https://libdccl.org/4.0/md_src_2doc_2markdown_2page02__idl.html#autotoc_md16) in the source Protobuf files (generally [jaiabot/src/lib/messages/jaia_dccl.proto](https://github.com/jaiarobotics/jaiabot/blob/3.y/src/lib/messages/jaia_dccl.proto)). An APIResponse will not be returned for queries of text or binary files. The file contents will be the body of the response.
...
"""
    ...
    for field in jaiabot.messages.rest_api_pb2.APIRequest.DESCRIPTOR.oneofs_by_name['action'].fields:
        request_section.add_subsections(generate_request_section(field.name))
    ...
    for field in jaiabot.messages.rest_api_pb2.APIResponse.DESCRIPTOR.oneofs_by_name['action'].fields:
        response_section.add_subsections(generate_response_section(field.name))
    ...
    for oneof_name, oneof_desc in oneofs.items():
        for oneof_field in oneof_desc.fields:
            ...
            if presence == jaiabot.messages.option_extensions_pb2.RestAPI.GUARANTEED:
                ...
                section.add_subsections(add_variant(f"Variant: {oneof_name} = {oneof_field.name}", jaia_response, action, oneof_selection))
