...
For messages to the MCU, the `jaiabot::sensor::protobuf::SensorRequest` Protobuf message is always used, and for message from the MCU, the `jaiabot::sensor::protobuf::SensorData` message is always used. Both are defined in `jaiabot/src/lib/messages/sensor/sensor_core.proto`.
...
The MCU is not expected to do anything until it receives a SensorRequest message from the `jaiabot_sensors` application. Once this message is received, it responds with the metadata for each sensor connected (one message per sensor). `jaiabot_sensors` uses this metadata message to launch the correct sensor driver threads. 
