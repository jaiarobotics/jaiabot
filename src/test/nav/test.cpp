...
BOOST_AUTO_TEST_CASE(crab_per_rudder_shifts_travel_heading_not_bow_heading)
{
    StateEstimatorConfig cfg;
    cfg.declination = 0.0;
    cfg.crab_per_rudder = deg_to_rad(-0.5); // 0.5 deg of crab per rudder-unit
    StateEstimator est(cfg);

    ImuSample imu;
    imu.time = 0.0;
    imu.quaternion = level_attitude(0.0);
    imu.gravity = gravity_report(level_attitude(0.0));
    ...
    est.handle_control(ControlSample{0.0, 40.0});
    est.handle_motor(MotorSample{0.0, 2200.0});
    ...
    for (int i = 1; i <= 100; ++i)
    {
        ...
        imu.time = t;
        est.handle_imu(imu);
        est.handle_motor(MotorSample{t, 2200.0});
        est.handle_control(ControlSample{t, 40.0});
    }

    const NavSolution s = est.solution();
    BOOST_REQUIRE(s.position_valid);
    // The bow still points north: crab only affects the direction of travel.
    BOOST_CHECK_SMALL(rad_to_deg(std::abs(wrap_pi(s.heading))), 1.0);
    BOOST_REQUIRE(s.course_over_ground.has_value());
    const double expected = wrap_two_pi(cfg.crab_per_rudder * 40.0);
    BOOST_CHECK_SMALL(rad_to_deg(std::abs(wrap_pi(*s.course_over_ground - expected))), 2.0);
}

BOOST_AUTO_TEST_CASE(stale_rudder_stops_contributing_to_crab)
{
    StateEstimatorConfig cfg;
    cfg.declination = 0.0;
    cfg.crab_per_rudder = deg_to_rad(-0.5);
    cfg.control_timeout = 2.0;
    StateEstimator est(cfg);
    ...
    // No further control messages: the rudder should be treated as stale well before the
    // end of this run (control_timeout is 2 s, the run is 10 s).
    for (int i = 1; i <= 100; ++i)
    {
        const double t = 0.1 * static_cast<double>(i);
        imu.time = t;
    ...
    BOOST_REQUIRE(s.position_valid);
    BOOST_REQUIRE(s.course_over_ground.has_value());
    BOOST_CHECK_SMALL(rad_to_deg(std::abs(wrap_pi(*s.course_over_ground))), 2.0);
}
