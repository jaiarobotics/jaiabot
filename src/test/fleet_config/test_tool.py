...
class CommandTest(unittest.TestCase):
    ...
    @needs_render_deps
    def test_generate_without_jaia_ip_uses_the_2y_tools(self):
        fake_bin = os.path.join(self.env.dir, "bin")
        os.remove(os.path.join(fake_bin, "jaia_ip"))
        calls = os.path.join(self.env.dir, "calls")
        for name, address in (("jaia", "10.23.7.11"), ("jaia-ip.py", "10.23.7.1")):
            with open(os.path.join(fake_bin, name), "w") as f:
                f.write('#!/bin/sh\necho "{} $*" >> {}\necho {}\n'.format(name, calls, address))
            os.chmod(os.path.join(fake_bin, name), 0o755)
        ...
        with open(calls) as f:
            self.assertEqual(f.read().splitlines(),
                             ["jaia ip h1f7", "jaia-ip.py addr --net wlan --fleet_id 7 --ipv4 --node gateway"])
