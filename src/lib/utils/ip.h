...
{
    if (s == "bot") return NodeType::bot;
    if (s == "hub") return NodeType::hub;
    if (s == "desktop") return NodeType::desktop;
    if (s == "gateway") return NodeType::gateway;
    if (s == "rpicam") return NodeType::rpicam;
...
{
    if (s == "wlan") return Network::wlan;
    if (s == "fleet_vpn") return Network::fleet_vpn;
    if (s == "vfleet_vpn") return Network::vfleet_vpn;
    if (s == "cloudhub_vpn") return Network::cloudhub_vpn;
    if (s == "cloudhub_eth") return Network::cloudhub_eth;
    if (s == "vfleet_eth") return Network::vfleet_eth;
    if (s == "vfleet_wlan") return Network::vfleet_wlan;
    if (s == "vpc") return Network::vpc;
...
inline boost::asio::ip::address_v4 ipv4_base(int fleet_id, Network net)
{
    ...
    switch (net)
    {
        ...
        default:
            throw std::invalid_argument("No IPv4 base for network: " + network_to_string(net));
    }
}
...
inline boost::asio::ip::address_v6 parse_ipv6(const std::string& s)
{
    ...
    auto addr = boost::asio::ip::make_address_v6(s, ec);
    if (ec)
                                              ...
                                              const std::string& ipv6_base_str = "")
{
    ...
    switch (net)
    {
        ...
        case Network::vfleet_eth:
        {
            // ipv6_base + 1*2^64: add 1 to byte index 8 (high byte of lower 64 bits)
            auto base = get_base_from_arg();
            auto bytes = base.to_bytes();
            uint32_t carry = 1;
            for (int i = 8; i >= 0 && carry > 0; --i)
            {
                uint32_t sum = static_cast<uint32_t>(bytes[i]) + carry;
                bytes[i] = static_cast<uint8_t>(sum & 0xFF);
                carry = sum >> 8;
            }
            return boost::asio::ip::address_v6(bytes);
        ...
        case Network::vfleet_wlan:
        {
            // ipv6_base + 2*2^64: add 2 to byte index 8
            auto base = get_base_from_arg();
            auto bytes = base.to_bytes();
            uint32_t carry = 2;
            for (int i = 8; i >= 0 && carry > 0; --i)
        ...
        default:
            throw std::invalid_argument("No IPv6 base for network: " + network_to_string(net));
    }
}
...
inline uint64_t ipv6_node_offset(NodeType node, int node_id)
{
    ...
    {
        case NodeType::hub:
            return static_cast<uint64_t>(node_id); // 0*2^16 + node_id
        case NodeType::bot:
            return static_cast<uint64_t>(1 * 65536 + node_id); // 1*2^16 + node_id
        case NodeType::desktop:
            return static_cast<uint64_t>(2 * 65536 + node_id); // 2*2^16 + node_id
        case NodeType::gateway:
            return 1; // node_id ignored
        case NodeType::rpicam:
            throw std::invalid_argument("rpicam node is not supported for ipv6");
    }
