...
// 'fN' suffix (e.g. "b4").
//
// The 'jaia_fleet_id' environmental variable is the primary source: it is set
// in the systemd units generated by config/gen/systemd.py, and in login shells
// by /etc/profile.d/jaia.sh (which reads /etc/jaiabot/jaia.env). Both are
// generated from the debconf database.
//
// Where neither applies - cron, 'ssh host command', scripts that never sourced
// a profile - fall back to parsing the hostname, which the jaiabot-embedded
// postinst sets from the same debconf answers as "<type><id>-fleet<fleet_id>".
