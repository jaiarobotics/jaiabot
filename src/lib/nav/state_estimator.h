  ...
  public:
    ...
    void handle_gnss(const GnssSample& sample)
    {
            ...
            *sample.speed_over_ground >= cfg_.min_velocity_update_speed)
        {
            ...
            const double heading = attitude_.heading();
    ...
    NavSolution solution() const
    {
        ...
        if (s.position_valid)
        {
            ...
            s.lat = geo[0];
            s.lon = geo[1];
            s.speed_over_ground = dead_reckoner_.speed_over_ground(attitude_.heading());
            s.course_over_ground = dead_reckoner_.course_over_ground(attitude_.heading());
