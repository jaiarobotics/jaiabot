...
# expects 'jaia-create-ansible-inventory.sh -b 1,2,3 -h 1,2' where -b is the comma separated list of bot IDs, and -h is the comma separated list of hub IDs
...
for b in "${bots_array[@]}"; do
    ip=$(jaia ip --query_type addr --node_type bot --ip_net ${net} --fleet_id ${fleet} --node_id ${b} --ip_version ipv${ipv})
...
for h in "${hubs_array[@]}"; do
    ip=$(jaia ip --query_type addr --node_type hub --ip_net ${net} --fleet_id ${fleet} --node_id ${h} --ip_version ipv${ipv})
