...
JCC_HUB_IP=$(jaia ip --query_type addr --node_type hub --ip_net cloudhub_vpn --fleet_id ${FLEET_ID} --node_id ${JCC_HUB_ID} --ip_version ipv6)

FLEET_ID_HEX=$(printf '%x\n' ${FLEET_ID})
VPC_CIDR_BLOCK=$(jaia ip --query_type net --ip_net vpc --fleet_id ${FLEET_ID} --ip_version ipv4)
# maps onto real fleet IP assignment
VIRTUALFLEET_WLAN_CIDR_BLOCK=$(jaia ip --query_type net --ip_net vfleet_wlan --fleet_id ${FLEET_ID} --ip_version ipv4)
CLOUDHUB_CIDR_BLOCK=$(jaia ip --query_type net --ip_net cloudhub_eth --fleet_id ${FLEET_ID} --ip_version ipv4)
CLOUDHUB_ID=30
CLOUDHUB_ETH_IP_ADDRESS=$(jaia ip --query_type addr --ip_net cloudhub_eth --fleet_id ${FLEET_ID} --node_type hub --node_id ${CLOUDHUB_ID} --ip_version ipv4)

# IPv6 address to use for VirtualFleet VPN (fd6e:cf0d:aefa:FLEET_ID_HEX::/64)
VIRTUALFLEET_VPN_NETWORK_IPV6=$(jaia ip --query_type net --ip_net vfleet_vpn --fleet_id ${FLEET_ID} --ip_version ipv6)
VIRTUALFLEET_VPN_CLIENT_IPV6=$(jaia ip --query_type addr --ip_net vfleet_vpn --fleet_id ${FLEET_ID} --node_type desktop --node_id 1 --ip_version ipv6)
VIRTUALFLEET_VPN_SERVER_IPV6=$(jaia ip --query_type addr --ip_net vfleet_vpn --fleet_id ${FLEET_ID} --node_type hub --node_id ${CLOUDHUB_ID} --ip_version ipv6)
# IPv6 address to use for CloudHub VPN (fd0f:77ac:4fdf:FLEET_ID_HEX::/64)
CLOUDHUB_VPN_NETWORK_IPV6=$(jaia ip --query_type net --ip_net cloudhub_vpn --fleet_id ${FLEET_ID} --ip_version ipv6)
CLOUDHUB_VPN_CLIENT_IPV6=$(jaia ip --query_type addr --ip_net cloudhub_vpn --fleet_id ${FLEET_ID} --node_type desktop --node_id 1 --ip_version ipv6)
CLOUDHUB_VPN_SERVER_IPV6=$(jaia ip --query_type addr --ip_net cloudhub_vpn --fleet_id ${FLEET_ID} --node_type hub --node_id ${CLOUDHUB_ID} --ip_version ipv6)
...
SUBNET_CLOUDHUB_IPV6=$(jaia ip --query_type net --ip_net cloudhub_eth --fleet_id ${FLEET_ID} --ip_version ipv6 --ipv6_base ${VPC_IPV6_BLOCK})
...
SUBNET_VIRTUALFLEET_WLAN_IPV6=$(jaia ip --query_type net --ip_net vfleet_wlan --fleet_id ${FLEET_ID} --ip_version ipv6 --ipv6_base ${VPC_IPV6_BLOCK})
